...
class FlowDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        u_u0_by_fx = (x - cx) / fx
        v_v0_by_fy = (y - cy) / fy
        depth1 = depth1 / np.sqrt(u_u0_by_fx * u_u0_by_fx + v_v0_by_fy * v_v0_by_fy + 1)
        depth2 = depth2 / np.sqrt(u_u0_by_fx * u_u0_by_fx + v_v0_by_fy * v_v0_by_fy + 1)
        depth1 = np.array(depth1/600.0).astype(np.float32)
        depth2 = np.array(depth2/600.0).astype(np.float32)
    ...
    def __init__(self, aug_params=None, root='/home/zhou/file8T/raft/FlyingThings3D', dstype='frames_cleanpass'):
        ...
        for cam in ['left']:
            for direction in ['into_future']:
                image_dirs = sorted(glob(osp.join(root, dstype, 'TESTthree/*/*')))
                image_dirs = sorted([osp.join(f, cam) for f in image_dirs])

                depth_dirs = sorted(glob(osp.join(root,'disparity/TESTthree/*/*')))
                depth_dirs = sorted([osp.join(f, cam) for f in depth_dirs])
        ...
        for cam in ['left']:
            for direction in ['into_future']:
                ...
                for idir, ddir in zip(image_dirs, depth_dirs):
                    ...
                    # print(images)
                    # flows = sorted(glob(osp.join(fdir, '*.pfm')) )
                    depths= sorted(glob(osp.join(ddir, '*.png')) )
                    # print(images,depths)
